...
mu = 0.02;
ectoCell = [(center(2) + 2*sqrt(1/(2*mu))):n 1:(center(1) - 2*sqrt(1/(2*mu)))];
mesoCell = tissue(~ismember(tissue,ectoCell));
...
for i = 1:N
    %% dynamic forces
    alphaMax = ParaTime(i,0.5*N,10);
    alpha = alphaMax*exp(-mu*distance.^2)+baseline;
%    gammaMax = ParaTime(i,0.7*N,1.0);
   % gamma = gammaMax*exp(-mu*distance.^2);
%   gamma(:) = gammaMax;
%    betaMax = 3/(exp(20/N*(i-0.5*N))+1)+2;
%    beta(:) = betaMax;
