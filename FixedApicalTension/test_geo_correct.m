...
adhesion = 10; % adhesion default:10
...
%% energy minimization 
    ...
    net_force = -Egrad;
    E(i) = Etot;
%     if ~isempty(find(isnan(net_force)))
%         break
%     end
%  if i == 6676
%      break
...
%     pos_temp = pos+disp_f;
%     r_temp = sqrt(pos_temp(1,:).^2 + pos_temp(2,:).^2);
%     sin_theta = pos(2,r_temp>r0)./sqrt(pos(1,r_temp>r0).^2 + pos(2,r_temp>r0).^2);
%     cos_theta = pos(1,r_temp>r0)./sqrt(pos(1,r_temp>r0).^2 + pos(2,r_temp>r0).^2);
%     net_force(1,r_temp>r0) = net_force(1,r_temp>r0).*sin_theta.^2 + net_force(2,r_temp>r0).*sin_theta.*cos_theta;
%     net_force(2,r_temp>r0) = net_force(1,r_temp>r0).*sin_theta.*cos_theta + net_force(2,r_temp>r0).*cos_theta.^2;
%     disp_f = dt/ita*net_force;
     pos = pos + disp_f;
    [ap_l,bas_l,lat_l]= cal_length(pos);
